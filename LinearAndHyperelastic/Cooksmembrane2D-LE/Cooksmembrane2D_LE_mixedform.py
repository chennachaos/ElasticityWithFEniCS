...
# stress tensor
sigma  = 2*mu*(epsilon-(1/d)*epsvol*I) + p*I

# Weak form
L = inner(sigma, grad(u_test))*dx + inner((epsvol - p*eps), p_test)*dx - dot(traction, u_test)*ds(2)

# derivative/Jacobian
dL = derivative(L, w, dw)


# solved as a nonlinear problem to ensure no issues in model setup.
# The solution should converge in one iteration.
...
# Output file setup
file_results = XDMFFile("Cooksmembrane2D-LE-nelem8.xdmf")
